(function(window){
		...
		for(var eIndex_=0;eIndex_<columnMappingElementsLength_;++eIndex_){
			...
			//select event
				...
				if(!Object.isNull(sBindDatasetName_) && !Object.isNull(sBindDisplayColumn_) && !Object.isNull(sBindDisplayColumn_)){
					JGDS("dataset",sBindDatasetName_).addSelectForAutoFill(element_, sBindDisplayColumn_, sBindValueColumn_);
				}
			}
			//label,etc event
			else{
	...
	$.widget("jg.JGDataset",{
		...
		,_dataset : null, dataset : function(){
			return this._dataset;
		},_create : function(){
			var that_ = this;
			var element_ = this.element;
			
			var datasetName_ = element_.attr("jg-dataset");
			
			if(!Object.isNull(datasetName_) && datasetName_.length !== 0)
				this._dataset = $.JGDS.get(datasetName_);
	...
	window.JGDS = (function(){
		var entry_ = $._JGDS[arguments[0]];
		if(entry_ === undefined){
			console.error(arguments[0]+" is undefined");
		}else if($.type(entry_) !== "function"){
			console.error(arguments[0]+" is not function");
		}else return entry_.apply($._JGDS,arguments.slice(1));
	});
	...
	$._JGDS = {
		...
		,_convertToIndex : function(selector_){
			...
			var dataset_ = this._datasetMap[this._convertToIndex(selector_)];
			
			if(dataset_ === undefined && $.type(selector_) === "string"){
				this._datasetMap.push(new JGDSEntry(selector_, new JGDataset(data_)));
				return dataset_;
			}
